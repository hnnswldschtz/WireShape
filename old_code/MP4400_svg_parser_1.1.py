...
# todo
#- python 3 port
#- straight lines and polygons must be converted in start/end points
#rather then being rasterized.
#- pen change support
#-paper size
...
def document_parse(file_name, outputPath):
    ...
    hpgl.write('IN;IW0,0,16160,11400;\nVS40;\nSP1;\n')#DIN A3
...
def circle_rasterizer(cx,cy,r1,r2):
  ...
  resolves given ccordinate pair of a circle center and a two radixesinto a rasterized set of coordinates of a cricle or ellipsoid.
...
def polygonPrint(points):
    #points=(123.23,123.23,546.4,456.67,486.46,456.5,768.99,45.9)
    ...
    #for i in range(0,p_length-1): # von hier bis
    #    print i
        ##floatingPoints[i] = float(points[i])

    for i in range(0,p_length-2,2): # von hier bis
        #print i
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"i:o:r:'new'h")
    except getopt.GetoptError:
        print ('-i <inputfile> -o <output DIR> -r <resolution> -n[ew]')
        sys.exit(2)
    for opt, arg in opts:
        if opt == "-h":
            print ('usage: -i <inputfile> -o <output DIR> -r <resolution> -n[ew]')
            sys.exit()
        elif opt in ("-i"):
			file_name=str(arg)
			k=file_name.rfind("/")
