    ...
    for i in children:
        ...
        if i.nodeName == "path":
           # print "%s, d= %s" % (i.nodeName, i.getAttribute('d'))
            ...
            lineto_rasterizer(line_xy['x1'],line_xy['y1'],line_xy['x2'],line_xy['y2'],True)
            hpgl.write('PU;\n')
            ...
            lineto_rasterizer(x1,y1,x1+width,y1,True)
...
def lineto_rasterizer(x1,y1,x2,y2,pdCmd=False):
    ...
    if (pdCmd): hpgl.write('PD;\n');
    if (rasterize):
        x_diff=x2-x1
        y_diff=y2-y1
        distance = Vdist(x1,y1,x2,y2)
        #distance=abs(sqrt(x_diff**2+y_diff**2))
        x_diff_count=0.0
        y_diff_count=0.0
        # print "sad",distance
        while Vdist(x1 + x_diff_count, y1+y_diff_count, x2, y2)>res:
...
def polygonPrint(points):
    ...
    p_length=len(points)
    lineto_rasterizer(float(points[0]), float(points[1]), float(points[2]), float(points[3]),True)
    for i in range(2,p_length-2,2): # run through list in bi steps and stop two before end
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == "-h":
            ...
            print ('setting a resolution with -r switchs on rasterizing on all standart HPGL commands')
